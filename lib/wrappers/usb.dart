...
@Native<Int32 Function(Pointer<Void>, Pointer<Pointer<Pointer<Void>>>)>(symbol: 'libusb_get_device_list')
external int enumerate(Pointer<Void> context, Pointer<Pointer<Pointer<Void>>> devices);

@Native<Void Function(Pointer<Pointer<Void>>, Int32)>(symbol: 'libusb_free_device_list')
external void release(Pointer<Pointer<Void>> devices, int unref);

@Native<Pointer<Void> Function(Pointer<Void>, Uint16, Uint16)>(symbol: 'libusb_open_device_with_vid_pid')
...
@Native<Int32 Function(Pointer<Void>, Pointer<Uint8>)>(symbol: 'libusb_get_device_descriptor')
external int describe(Pointer<Void> device, Pointer<Uint8> output);

@Native<Int32 Function(Pointer<Void>, Pointer<Void>, Uint8, Uint16, Pointer<Uint8>, Int32)>(symbol: 'libusb_get_string_descriptor_ascii')
external int label(Pointer<Void> context, Pointer<Void> handle, int index, int lang, Pointer<Uint8> buffer, int length);
...
@Native<Int32 Function(Pointer<Void>, Pointer<Pointer<Void>>)>(symbol: 'libusb_open')
external int attach(Pointer<Void> device, Pointer<Pointer<Void>> output);

@Native<Int32 Function(Pointer<Void>, Uint8, Pointer<Uint8>, Int32, Pointer<Int32>, Uint32)>(symbol: 'libusb_bulk_transfer')
external int transfer(Pointer<Void> handle, int endpoint, Pointer<Uint8> buffer, int length, Pointer<Int32> sent, int timeout);

final DynamicLibrary library = () {
  if (Platform.isIOS)     throw UnsupportedError('iOS blocks raw USB access.');
  if (Platform.isFuchsia) throw UnsupportedError('Fuchsia is not supported yet.');
  ...
  return [
    ...
    try { return DynamicLibrary.open(path); } catch (_) { return null; }
...
Pointer<Void> boot() {
  return using((arena) {
    ...
    if (initialize(pointer) != 0) throw StateError('Failed to initialize libusb.');
...
class USB {
        ...
        final devices = List.generate(count, (index) {
          ...
          String? read(int stringIndex) {
            ...
            return using((scope) {
              ...
              final length = label(context, opened.value, stringIndex, 0x0409, buffer, 256);
  ...
  List<int> pull(int endpoint, int length, {int timeout = 1000}) {
    return using((arena) {
      ...
      if (transfer(handle, endpoint, buffer, length, transferred, timeout) < 0) {
        throw StateError('Read from endpoint 0x${endpoint.toRadixString(16)} failed.');
      }
  ...
  int push(int endpoint, List<int> bytes, {int timeout = 1000}) {
    return using((arena) {
      ...
      if (transfer(handle, endpoint, buffer, bytes.length, transferred, timeout) < 0) {
        throw StateError('Write to endpoint 0x${endpoint.toRadixString(16)} failed.');
      }
